...
<div class="container">
  <div class="w-full max-w-sm p-4 my-28 bg-white border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 ">
    <form class="space-y-6" method="POST" action="{{ route('user.store') }}">
      @csrf
      ...
      <div>
        ...
        <input type="file" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none" id="file_input" required />
      </div>
